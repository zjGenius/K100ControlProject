...
static std::vector<std::complex<double>> generateSingalF(double sample_fs = 2048e3, int sample_point_size = 4096, double delta_f = 1, double times = 1.0)
{
    ...
    printf("sample_resolution = %fHz , delta_f = %.3fMHZ\n", sample_resolution, delta_f * sample_resolution / 1000000.0);
    delta_f /= times;
    //    printf("times = %f\n",times);
...
{
    if(delta_f_list.size() <= 1){//频点列表小于2不处理
        return sample_data;
    }
    ...
    for (int k = 0; k < delta_f_list.size(); k++) {
        auto conversion_data_f = generateSingalF(conversion_fs, conversion_size, 1000000.0*delta_f_list[k] / conversion_resolution, conversion_per);
        for (int i = 0; i < conversion_size; i++) {
