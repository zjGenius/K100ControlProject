...
public:
    ...
    bool get_keys(unsigned int &value, int ch = 0);
...
private:
    ...
    bool _get_keys(unsigned int &value, int chid)
    {
        auto ctx = this->ctx_->context();
        char name[64]{0};
        snprintf(name, sizeof(name), "altvoltage%d", chid);
        ...
        char _tmp_value[16];
        int ret = iio_channel_attr_read(chn, "keys", _tmp_value, sizeof(_tmp_value));
        if (ret < 0)
        {
            char buf[256];
            iio_strerror(errno, buf, sizeof(buf));
...
private:
    ...
    // Rx
    std::atomic<std::int32_t> rx_fc;
    std::atomic<std::int32_t> rx_bw;
    std::atomic<std::int32_t> rx_fs;
    ...
    // thread
    std::atomic<bool> rx_stop_;
    std::thread rx_thread_;

    std::atomic<bool> tx_stop_;
